...
function App() {
  ...
  useEffect(() => {
    getPeople(page)
      .then(setPeople)
      .catch(handlerError);
  }, [page]); //array de dependencias que si se pone vacio lo ejecutara solo una vez al principio de la carga

  useEffect(() => {
    getCharacter(currentCharacter).then(setDetails).catch(handlerError);
  }, [currentCharacter]);
